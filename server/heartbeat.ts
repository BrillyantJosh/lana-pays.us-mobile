...
export async function runHeartbeat(db: Database.Database): Promise<void> {
  ...
  try {
    ...
    if (latest38888 && latest38888.event_id === systemParams.event_id) {
      ...
      db.prepare(
        "UPDATE kind_38888 SET split_approaching = ?, freeze_lana_retail_account_above = ?, max_cap_lanas_on_split = ?, updated_at = datetime('now') WHERE id = ?"
      ).run(systemParams.split_approaching ? 1 : 0, systemParams.freeze_lana_retail_account_above || 0, systemParams.max_cap_lanas_on_split || 0, latest38888.id);
      console.log(`[sync] KIND 38888 unchanged (${systemParams.event_id.slice(0, 12)}…), refreshed v3 columns`);
    } else {
      db.prepare(`
        INSERT INTO kind_38888 (event_id, split, exchange_rates, electrum_servers, relays, version, valid_from, split_target_lana, split_started_at, split_ends_at, split_approaching, freeze_lana_retail_account_above, max_cap_lanas_on_split, raw_event, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
      `).run(
        ...
        systemParams.max_cap_lanas_on_split || 0,
